...
- **Weather Provider client** (deep) — interface: `Location → (Current Conditions + 7-day daily Forecast)`. Wraps Open-Meteo's forecast endpoint (`api.open-meteo.com`) requesting the current block plus the daily array, with **fixed metric unit params** (`temperature_unit=celsius`, `wind_speed_unit=kmh`). Returns the domain weather model; never exposes raw provider JSON upward. Coordinates are formatted with `CultureInfo.InvariantCulture` so the decimal separator is always `.` regardless of host locale (a comma-decimal host must not emit `latitude=51,5`), and every call is bounded by a **finite timeout linked to the caller's `CancellationToken`** so a hung endpoint or a cancelled caller fails closed to the load-failure path rather than hanging. *Implemented so far:* `OpenMeteoWeatherProvider.GetCurrent(Location, CancellationToken) → CurrentConditions` requesting the `current` block only (`temperature_2m,weather_code,wind_speed_10m`, `weather_code` routed through the WMO map); the daily-Forecast half of the contract is **Feature 2** (see `Roadmap.md`).
...
- **Search ViewModel** — orchestrates a **Location Search**: invokes the Geocoder, exposes the candidate list, handles the zero-results message (preserving current state), and on explicit selection sets the active Location and triggers a weather fetch. *Implemented so far:* `SearchViewModel` (CommunityToolkit.Mvvm `ObservableObject`) owning `Query`, `Candidates`, a `SearchMessage` line and `SelectCommand`, raising `LocationSelected(Location)` on explicit select. A **300 ms debounce** through an injected `IDebounceScheduler` seam collapses rapid typing to one Geocoder call; a query of **fewer than 2 characters** clears candidates and makes no call. Each run takes a monotonically increasing `searchSeq`, cancels the prior in-flight token, and applies results **only if still the latest** (guard-on-arrival) so out-of-order responses can't render stale candidates — the guard's shared counter is fenced with `Interlocked.Increment` / `Volatile.Read` because debounced continuations resume on arbitrary ThreadPool threads. Any Geocoder failure surfaces a **fixed neutral line** (never the exception text/stack trace or the request URL, per Technical-Context). The activation half of the contract — turning a `LocationSelected` into an active Location and a fresh fetch — lives in the **MainViewModel shell** (below), which is the sole subscriber to `LocationSelected`; the Search ViewModel raises the event and never references the Weather side. The debounce clock (`DebounceScheduler`, a `System.Timers.Timer` marshalling onto the captured `SynchronizationContext`) is the injected seam that lets Tier-1 tests fire synchronously.
- **Weather ViewModel** — on activation (selection or launch-restore) and on manual refresh, fetches fresh from the Weather Provider client; holds Current Conditions, the 7-day Forecast, the **Updated-at** stamp, and loading/error state. Applies the rules: always fetch fresh; on refresh failure keep last-good + Updated-at; on fresh-load failure show error + retry. *Implemented so far:* `WeatherViewModel` (CommunityToolkit.Mvvm `ObservableObject`) holding **Current Conditions only** and driving a fresh-fetch load state machine — `Load(Location)` always fetches fresh from the injected `IWeatherProvider` (ADR-0001) and moves a `WeatherLoadState` enum `Idle → Loading → Loaded` on success or `Loading → Error` on failure, exposing `Conditions`, `LocationName`, `State` and `ErrorMessage`. A provider failure surfaces a **fixed neutral line** only (never the exception text/stack trace or the request URL, which carries the Location's coordinates, per Technical-Context). Still to arrive: the 7-day Forecast, the **Updated-at** stamp, refresh-failure keep-last-good, and the retry affordance.
- **MainViewModel (shell)** — composes the two child ViewModels and mediates **Activation** between them, following **Approach A (shell-mediated composition)**: the shell owns `Search` and `Weather` and is the **sole subscriber** to `SearchViewModel.LocationSelected`; the children **never reference each other**. On selection it flips a `ViewState` (`WeatherViewState` enum: `Empty` → `Weather`) and calls `WeatherViewModel.Load(location)`. It starts in `Empty` (the first-run search prompt) and exposes the in-flight load as an awaitable `LastActivation`. *Why Approach A over child-to-child wiring:* keeping the search and weather sides mutually ignorant lets each be tested and re-composed in isolation, and concentrates the activation rule (select → activate → fresh fetch) in one place. *Not yet wired:* launch-restore activation (the other Activation trigger) arrives with the Location Store; the shell today is driven only by explicit selection.
- **Views (XAML)** — empty state with search prompt (first run); search box + candidate list; weather view (Current Conditions + 7-day daily strip + Updated-at); inline status line for errors and loading. *Implemented so far:* the WPF shell is on the **.NET generic host** (`App.xaml.cs`, `Host.CreateDefaultBuilder`) with DI + logging as the composition root; `App.xaml` carries **no StartupUri** — `OnStartup` resolves `MainWindow` from the container, sets its `DataContext` to a resolved `MainViewModel`, and shows it. DI registers **two typed HttpClients** with distinct **`https://`** base hosts (Geocoder → `geocoding-api.open-meteo.com`, Weather Provider → `api.open-meteo.com`; no TLS-validation weakening), the WMO map, the debounce scheduler, the three ViewModels and `MainWindow`. `MainWindow.xaml` gives an always-visible search box, a candidate list + search-message panel bound to `Search` (double-click raises `SelectCommand`), and a body that switches the **Empty** prompt vs the **Weather** view off `ViewState`, the Weather view showing temperature (°C), Condition, wind (km/h) and Loading/Error states via a set of small `IValueConverter`s. `MainWindow.xaml.cs` is `InitializeComponent` only — all state lives in the ViewModels. Still to arrive: the 7-day daily strip and Updated-at (they follow the Weather ViewModel's later slices).
...
- **Search ViewModel** — Tier-1 unit tests with the Geocoder faked and a **manual (fake) debounce clock**: debounce collapses rapid typing to one call, a sub-2-character query makes no call, the sequence guard drops stale out-of-order responses (latest query wins), zero-results yields the "No places found" copy, a Geocoder failure yields the fixed neutral line only (no exception text / URL), and `SelectCommand` raises `LocationSelected` with the correct `Location`.
...
**Tiers / matrix:** Tier-1 (recorded-replay) on every commit; Tier-2 (live Open-Meteo, bounded/scheduled) confirms the recorded fixtures still match the real contract; platform matrix is **Windows** (the OS-touching code is the Location Store's local persistence). Coverage is planned at the Feature level per the testing standard. *Implemented so far:* the Tier-2 tier is realised by `OpenMeteoLiveTests` (`tests/WeatherApp.Tests/Live/`) — one live call each to the geocoding and forecast endpoints, asserting on the deterministic envelope (candidate name/country present; a non-empty Condition) and never on volatile weather values. The class carries the `Tier=Live` trait, so every-commit runs use `dotnet test --filter Tier!=Live` and the scheduled live run uses `--filter Tier=Live`.
...
- This PRD scopes the product as a whole; `/roadmap` split the grilled "search → Current Conditions + 7-day daily Forecast" slice: current weather landed as **Feature 1**, and the 7-day daily Forecast is **Feature 2** (feature ownership and ordering live in `Roadmap.md`). Hourly Forecast and a units preference are the natural follow-on Features.
